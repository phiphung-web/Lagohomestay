...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimit(`booking:${forwarded}`, 6, 60_000).allowed)
    return NextResponse.json(
      { message: "Bạn thao tác hơi nhanh. Vui lòng thử lại sau một phút." },
      { status: 429 },
    );
  ...
  if (!parsed.success)
    return NextResponse.json(
      { message: parsed.error.issues[0]?.message ?? "Thông tin đặt phòng chưa hợp lệ" },
      { status: 400 },
    );
  ...
  if (process.env.DEMO_MODE !== "false" || !process.env.DATABASE_URL) {
    ...
    if (!stay || data.guests > stay.maxGuests)
      return NextResponse.json(
        { message: "Không gian này không phù hợp với số khách đã chọn." },
        { status: 409 },
      );
    ...
    if (!booking)
      return NextResponse.json(
        { message: "Phòng vừa được khách khác giữ. Vui lòng chọn không gian hoặc ngày khác." },
        { status: 409 },
      );
    return NextResponse.json(
      { holdExpiresAt: booking.holdExpiresAt, totalAmount: booking.totalAmount },
      { status: 201 },
    );
  }
  ...
  try {
    ...
    const result = await prisma.$transaction(
      async (tx) => {
        ...
        await tx.booking.updateMany({
          where: { status: BookingStatus.HELD, holdExpiresAt: { lte: new Date() } },
        ...
        const unit = await tx.unit.findUnique({
          ...
          include: {
            accommodationType: {
              include: { rateRules: { where: { active: true } }, property: true },
            },
          },
        });
        ...
        const rules = unit.accommodationType.rateRules.map((rule) => ({
        ...
        const quote = calculatePrice({
          ...
          checkOut,
          guests: data.guests,
          baseGuests: unit.accommodationType.baseGuests,
          basePrice: Number(unit.accommodationType.basePrice),
          rules,
        });
        ...
        const holdExpiresAt = addMinutes(
          new Date(),
          unit.accommodationType.property.defaultHoldMinutes,
        );
        const booking = await tx.booking.create({
          data: {
            ...
            guests: data.guests,
            totalAmount: quote.total,
            priceSnapshot: quote as unknown as Prisma.InputJsonValue,
            guestNote: data.note,
            holdExpiresAt,
            idempotencyKey,
          },
    ...
    return NextResponse.json(
      { holdExpiresAt: result.holdExpiresAt, totalAmount: Number(result.totalAmount) },
      { status: 201 },
    );
  } catch (error) {
    const reason = error instanceof Error ? error.message : "";
    if (["NOT_AVAILABLE", "UNIT_NOT_FOUND", "CAPACITY_EXCEEDED"].includes(reason))
      return NextResponse.json(
        {
          message:
            reason === "NOT_AVAILABLE"
              ? "Phòng vừa được khách khác giữ. Vui lòng chọn không gian hoặc ngày khác."
              : "Không gian này hiện không thể đặt.",
        },
        { status: 409 },
      );
    ...
    return NextResponse.json(
      {
        message:
          "LAKA chưa thể ghi nhận yêu cầu. Vui lòng thử lại hoặc gọi trực tiếp cho chúng mình.",
      },
      { status: 500 },
    );
  }
