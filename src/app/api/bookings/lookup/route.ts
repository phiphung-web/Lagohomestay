...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimit(`lookup:${ip}`, 10, 60_000).allowed)
    return NextResponse.json(
      { message: "Bạn đã kiểm tra quá nhiều lần. Vui lòng thử lại sau." },
      { status: 429 },
    );

  const parsed = lookupBookingSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success)
    return NextResponse.json({ message: "Số điện thoại chưa đúng định dạng." }, { status: 400 });
  ...
  if (!bookings.length)
    return NextResponse.json(
      { message: "Không tìm thấy yêu cầu đặt chỗ với số điện thoại này." },
      { status: 404 },
    );
