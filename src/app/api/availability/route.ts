...
export async function GET(request: NextRequest) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      { message: parsed.error.issues[0]?.message ?? "Thông tin ngày ở chưa hợp lệ" },
      { status: 400 },
    );
  ...
  if (process.env.DEMO_MODE !== "false" || !process.env.DATABASE_URL) {
    const results = stays
      ...
      .map((stay) => {
        const availableUnits = getUnitsForStay(stay.id).filter((unit) =>
          isDemoUnitAvailable(unit.id, checkIn, checkOut),
        );
        ...
        return {
          ...
          quote: calculatePrice({
            ...
            basePrice: stay.basePrice,
            rules: demoRules.map((rule) => ({
              ...rule,
              minGuests: rule.type === "EXTRA_GUEST" ? stay.baseGuests + 1 : undefined,
            })),
          }),
        };
      })
  ...
  try {
          ...
          where: {
            ...
            bookings: {
              none: {
                ...
                checkOut: { gt: start },
                status: {
                  in: [BookingStatus.HELD, BookingStatus.CONFIRMED, BookingStatus.CHECKED_IN],
                },
              },
            },
    ...
    const results = types
      ...
      .map((type) => {
        ...
        const highlights =
          concept?.highlights ??
          (Array.isArray(type.amenities) ? (type.amenities as string[]).slice(0, 3) : []);
        return {
          id: type.id,
    ...
    console.error("availability_lookup_failed", {
      reason: error instanceof Error ? error.message : "unknown",
    });
    return NextResponse.json(
      { message: "LAKA chưa thể kiểm tra lịch lúc này. Vui lòng thử lại sau ít phút." },
      { status: 500 },
    );
  }
}
