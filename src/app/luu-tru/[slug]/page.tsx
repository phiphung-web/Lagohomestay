...
export default async function StayDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <SiteShell>
    ...
    <section className="pb-24"><div className="container-lago"><div className="mb-8 flex items-end justify-between"><div><p className="eyebrow text-lago-clay">Nhìn gần hơn</p><h2 className="display mt-2 text-4xl font-semibold sm:text-5xl">Không gian của {stay.name}</h2></div><span className="hidden text-xs font-bold uppercase tracking-wider text-lago-ink/40 sm:block">Ảnh concept minh họa</span></div><div className="grid gap-4 sm:grid-cols-12">{stay.gallery.map((image, index) => <div key={image} className={`image-zoom relative overflow-hidden rounded-[26px] ${index === 0 ? "aspect-[16/10] sm:col-span-8" : index === 1 ? "aspect-square sm:col-span-4" : "aspect-[16/8] sm:col-span-12"}`}><Image src={image} alt={`Không gian ${stay.name} ${index + 1} - ảnh concept`} fill sizes={index === 2 ? "100vw" : "60vw"} className="object-cover" /></div>)}</div></div></section>
