      ...
      <header className="sticky top-0 z-50 border-b border-lago-ink/10 bg-[#fbfaf6]/90 backdrop-blur-xl">
        <div className="container-lago flex h-[76px] items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/lien-he" className="btn-primary rounded-full px-4 py-2 text-xs sm:text-sm">Đặt ngay</Link>
          </div>
        </div>
        {open && <div className="border-t border-lago-ink/10 bg-[#fbfaf6] px-5 py-5 lg:hidden">
          <nav className="flex flex-col gap-1">{links.map(([href, label]) => <Link onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 font-semibold" key={href} href={href}>{label}</Link>)}<Link onClick={() => setOpen(false)} href="/lien-he" className="btn-primary mt-3 w-full">Đặt ngay</Link></nav>
        </div>}
      </header>
    </>
  );
