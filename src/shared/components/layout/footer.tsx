...
export function Footer() {
  return <footer className="bg-lago-ink text-white">
    <div className="container-lago grid gap-12 py-16 md:grid-cols-[1.4fr_.8fr_.8fr]">
      ...
      <div><p className="eyebrow text-white/45">Khám phá</p><div className="mt-5 flex flex-col gap-3 text-sm"><Link href="/luu-tru">Bộ sưu tập căn nhà</Link><Link href="/trai-nghiem">Trải nghiệm tại LAKA</Link><Link href="/thong-tin">Câu hỏi thường gặp</Link><Link href="/chinh-sach">Chính sách lưu trú</Link><Link href="/dat-phong">Tư vấn chọn căn</Link><Link href="/tra-cuu">Tra cứu yêu cầu</Link></div></div>
      <div><p className="eyebrow text-white/45">Kết nối</p><div className="mt-5 flex flex-col gap-4 text-sm"><a href={publicContact.phoneHref} className="flex gap-3"><Phone className="h-4 w-4"/>{publicContact.phoneDisplay}</a><a href={publicContact.zaloHref} className="flex gap-3">Zalo đặt phòng <ArrowUpRight className="h-4 w-4"/></a><a href={publicContact.emailHref} className="flex gap-3"><Mail className="h-4 w-4 shrink-0"/>{publicContact.email}</a><span className="flex gap-3 text-white/65"><MapPin className="h-4 w-4 shrink-0"/>{publicContact.address}</span><span className="flex gap-3"><Instagram className="h-4 w-4"/>@lagohomestay</span></div></div>
    </div>
