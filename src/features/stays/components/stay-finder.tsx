    ...
    <div className="grid lg:grid-cols-[.92fr_1.08fr]">
      <div className="p-6 sm:p-9 lg:p-11">
        ...
        <div className="mt-8 space-y-7">
          ...
          <fieldset><legend className="mb-3 text-xs font-bold uppercase tracking-wider text-white/45">2 · Điều bạn muốn nhất?</legend><div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">{vibes.map(({ value, label, icon: Icon }) => <button type="button" key={value} onClick={() => setVibe(value)} className={`focus-ring flex shrink-0 items-center gap-2 rounded-full border px-4 py-2.5 text-xs font-bold transition ${vibe === value ? "border-lago-sand bg-lago-sand text-lago-ink" : "border-white/15 text-white/65 hover:border-white/30"}`}><Icon className="h-4 w-4" />{label}</button>)}</div></fieldset>
