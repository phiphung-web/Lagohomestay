...
export function MainHome({
  ...
  const en = locale === "en";
  const heroHeadline = en
    ? "Choose a cabin - Hold the whole valley"
    : '"Chọn" Cabin - "Trọn" Thung Lũng';
  const [heroFirstLine, heroSecondLine] = heroHeadline.split(" - ");

  return (
    <div className="showcase-root min-h-screen bg-[#eae1d2] text-[#16311c]">
      <DocumentLocale locale={locale} />
      <SkipLink label={en ? "Skip navigation" : "Bỏ qua điều hướng"} />
      <ExperienceLayer mood="editorial" />
        ...
        <section className="relative min-h-[100svh] overflow-hidden bg-[#10251d] text-white">
          <picture className="absolute inset-0 block">
            ...
            <img
              ...
              alt={
                en ? "LAKA Homestay among the green landscape" : "LAKA Homestay giữa khoảng xanh"
              }
              width={2560}
              height={1440}
          ...
          <div className="relative z-10 mx-auto flex min-h-[100svh] w-[min(1500px,calc(100%-32px))] flex-col justify-end pb-16 pt-36 sm:w-[min(1500px,calc(100%-56px))] sm:pb-20">
            ...
            <div className="mt-8 flex items-end justify-between gap-8 border-t border-white/22 pt-6">
              ...
              <a
                href="#gioi-thieu"
                className="focus-ring group hidden items-center gap-3 text-[.6rem] font-bold uppercase tracking-[.18em] text-white/65 sm:flex"
              >
                {en ? "Discover LAKA" : "Khám phá LAKA"}
                <span className="grid h-11 w-11 place-items-center rounded-full border border-white/30 transition group-hover:translate-y-1 group-hover:bg-white group-hover:text-[#16311c]">
            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[#16311c]/15 pb-8 sm:pb-10">
              <div>
                <h2 className="laka-home-section-title text-[#16311c]">
                  {en ? "Memory gallery" : "Thư viện ký ức"}
                </h2>
              </div>
              <p className="font-serif text-2xl sm:text-3xl lg:text-4xl font-medium text-[#16311c] md:text-right shrink-0">
                {en ? (
                  <>
                    ...
                    <i className="text-[#80613f]">the days worth remembering.</i>
                  </>
                ) : (
                  <>
                    Những khung hình lưu lại
                    <br className="hidden md:inline" />{" "}
                    <i className="text-[#80613f]">một ngày thật đáng nhớ.</i>
                  </>
                )}
              </p>
            </header>
      ...
      <SiteFooter config={config} locale={locale} homeMode />
    </div>
  );
}
