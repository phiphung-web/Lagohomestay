...
export function TemplateFooter({ config, locale = "vi" }: { config: CompleteTemplateConfig; locale?: ShowcaseLocale }) {
  ...
  return <footer className={`border-t border-current/12 pb-28 pt-14 sm:pb-32 ${config.mood === "organic" ? "bg-[#e7ded1]" : config.mood === "cinematic" ? "bg-[#0b190f]" : "bg-[#faf3ea]"}`}>
    <div className="mx-auto grid w-[min(1420px,calc(100%-40px))] gap-10 md:grid-cols-[1.1fr_.7fr_.7fr]">
      <div><Link href={config.basePath} aria-label={locale === "en" ? "LAKA Homestay - Concept home" : "LAKA Homestay - Trang chủ mẫu"} className="inline-flex"><BrandLogo variant={config.mood === "editorial" ? "established" : "homestay"} decorative className={`${config.mood === "editorial" ? "w-[190px]" : "w-[210px]"} ${config.mood === "cinematic" ? "text-[#faf3ea]" : "text-[#17321d]"}`} /></Link><p className="mt-5 max-w-md text-sm leading-7 opacity-80">{locale === "en" ? "Four private homes surrounded by nature, made for slower days and more meaningful time together." : "Bốn căn nhà riêng giữa thiên nhiên, cho những ngày mọi người muốn sống chậm và gần nhau hơn."}</p><span className="mt-5 inline-flex rounded-full border border-current/15 px-3 py-1.5 text-[.6rem] font-bold uppercase tracking-wider opacity-80">{locale === "en" ? "Presentation · Concept" : "Bản trình bày · Mẫu"} {config.name}</span></div>
...
function StaysPage({ config, locale }: { config: CompleteTemplateConfig; locale: ShowcaseLocale }) {
  return <><PageIntro config={config} locale={locale} eyebrow={locale === "en" ? "The private home collection" : "Bộ sưu tập căn riêng"} title={locale === "en" ? "Four homes, four ways to slow down." : "Bốn căn nhà, bốn nhịp nghỉ riêng."} text={locale === "en" ? "Choose by who you travel with, the length of your stay and the view you want to wake up to." : "Chọn căn theo người đồng hành, số ngày ở và cách bạn muốn thức dậy mỗi sáng."} image={conceptImages.detail1} />
...
function BookingPrelude({ config, locale }: { config: CompleteTemplateConfig; locale: ShowcaseLocale }) {
  ...
  return <div className="border-b border-[#17321d]/12 bg-[#e7ded1] px-6 py-16 text-[#17321d] sm:px-10 sm:py-20"><div className="mx-auto grid max-w-[1240px] gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-end"><div><p className="text-[.62rem] font-bold uppercase tracking-[.2em] text-[#80613f]">{locale === "en" ? "LAKA journal · Stay dates" : "LAKA ký sự · Lịch lưu trú"}</p><p className="mt-12 max-w-xs text-sm leading-7 text-[#17321d]/72">{locale === "en" ? "Choose enough time to truly rest, then let LAKA find the home that fits." : "Chọn một khoảng thời gian đủ rộng để nghỉ ngơi, rồi để LAKA tìm căn phù hợp nhất."}</p></div><p className="font-serif text-5xl font-medium leading-[.98] tracking-[-.05em] sm:text-7xl">{locale === "en" ? <>Keep a place<br /><i>entirely your own.</i></> : <>Giữ lại một khoảng<br /><i>thật riêng cho bạn.</i></>}</p></div></div>;
}

function BookingPage({ config, locale }: { config: CompleteTemplateConfig; locale: ShowcaseLocale }) {
  return <section className="min-h-screen bg-[var(--template-bg)] px-0 py-0 text-[#17321d] sm:px-4 sm:py-8"><div className={`mx-auto max-w-[1480px] overflow-hidden bg-[#fbfaf6] ${config.mood === "organic" ? "sm:rounded-[38px]" : config.mood === "cinematic" ? "border border-white/10" : "border border-[#17321d]/10"}`}><BookingPrelude config={config} locale={locale} /><Suspense fallback={<div className="container-lago py-24">{locale === "en" ? "Preparing availability…" : "Đang chuẩn bị lịch căn nhà…"}</div>}><BookingExperience lookupPath={scoped(config.basePath, "tra-cuu")} locale={locale} /></Suspense></div></section>;
}
...
function LookupPage({ config, locale }: { config: CompleteTemplateConfig; locale: ShowcaseLocale }) {
  ...
  return <section className="min-h-[75svh] border-b border-[#17321d]/12 bg-[#e7ded1] py-16 text-[#17321d] sm:py-24"><div className="mx-auto grid w-[min(1240px,calc(100%-40px))] gap-12 lg:grid-cols-[1fr_480px] lg:items-center"><div><p className="text-[.62rem] font-bold uppercase tracking-[.2em] text-[#80613f]">{locale === "en" ? "LAKA journal · Your stay" : "LAKA ký sự · Thông tin chuyến đi"}</p><h1 className="mt-6 max-w-3xl font-serif text-5xl font-medium leading-[.98] tracking-[-.05em] sm:text-7xl">{locale === "en" ? <>Find the place<br /><i>waiting for you.</i></> : <>Xem lại khoảng riêng<br /><i>đang chờ bạn.</i></>}</h1><p className="mt-7 max-w-xl text-sm leading-7 text-[#17321d]/72">{locale === "en" ? "Enter the phone number used for your request. LAKA will show the most recent bookings linked to it." : "Chỉ cần nhập số điện thoại đã dùng khi đặt. LAKA sẽ hiển thị các yêu cầu gần nhất gắn với số này."}</p><div className="mt-8 max-w-xl border-l border-[#80613f]/45 pl-5 text-sm leading-6 text-[#17321d]/72">{locale === "en" ? "No booking code required. For immediate assistance, call " : "Không cần nhớ mã đặt chỗ. Nếu cần hỗ trợ ngay, gọi "}<a href="tel:0900000000" className="font-bold underline underline-offset-4">0900 000 000</a>.</div></div><div className="rounded-t-[150px] border border-[#17321d]/12 bg-[#faf3ea] px-6 pb-8 pt-24 text-[#17321d] shadow-xl sm:px-8"><LookupForm locale={locale} /></div></div></section>;
}
