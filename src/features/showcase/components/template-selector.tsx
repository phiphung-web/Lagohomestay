    ...
    <div className="mx-auto max-w-[1500px]">
      ...
      <section className="pb-16 sm:pb-24">
        <div className="overflow-hidden rounded-[28px] border border-white/12">
          <div className="grid grid-cols-[72px_1fr] border-b border-white/10 bg-white/[.045] px-5 py-4 text-[.62rem] font-bold uppercase tracking-[.15em] text-white/35 sm:grid-cols-[110px_.55fr_1fr_1fr]"><span>Mẫu</span><span className="hidden sm:block">Dành cho</span><span>Dấu ấn thị giác</span><span className="hidden sm:block">Cảm giác tương tác</span></div>
          {showcaseTemplates.map((template) => <Link href={`/mau/${template.slug}`} key={template.slug} className="group grid grid-cols-[72px_1fr] gap-y-3 border-b border-white/10 px-5 py-6 transition last:border-b-0 hover:bg-white/[.05] sm:grid-cols-[110px_.55fr_1fr_1fr] sm:items-center">
            <span className="font-serif text-xl font-medium text-[#e5c59c]">{template.number}</span><span className="hidden text-xs leading-5 text-white/48 sm:block">{template.audience}</span><span className="pr-4 text-sm font-semibold leading-6">{template.signature}</span><span className="hidden items-center justify-between gap-4 text-xs leading-6 text-white/45 sm:flex">{template.interaction}<ArrowRight className="h-4 w-4 shrink-0 text-[#e5c59c] transition group-hover:translate-x-1" /></span>
          </Link>)}
        </div>
      </section>
    </div>
