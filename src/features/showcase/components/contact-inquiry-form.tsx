...
export function ContactInquiryForm({ locale = "vi" }: { locale?: ShowcaseLocale }) {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const subjectBase = en ? "Contact request from LAKA website" : "Yêu cầu liên hệ từ website LAKA";
    const subject = selectedStayName ? `${subjectBase} · ${selectedStayName}` : subjectBase;
    const body = [
      ...
      `${en ? "Stay of interest" : "Căn quan tâm"}: ${selectedStayName || "—"}`,
      `${en ? "Expected dates" : "Ngày dự kiến"}: ${data.get("checkIn") || "—"} → ${data.get("checkOut") || "—"}`,
      `${en ? "Guests" : "Số khách"}: ${data.get("guests") || "—"}`,
      "",
  ...
  return <section id="inquiry-form" aria-labelledby="inquiry-form-title" className="scroll-mt-24 border-t border-[#16311c]/12 bg-[#fbfaf6] py-20 text-[#16311c] sm:py-28">
    <div className="mx-auto grid w-[min(1180px,calc(100%-40px))] gap-12 lg:grid-cols-[.7fr_1fr] lg:items-start">
      <header className="lg:sticky lg:top-32">
        ...
        <p className="laka-body-muted mt-6 max-w-md">{en ? "Stay advice and every other request are handled here. The form opens your email app with the details ready for review before sending." : "Tư vấn lưu trú và mọi nhu cầu khác đều được tiếp nhận tại đây. Form sẽ mở ứng dụng email với nội dung đã điền để bạn kiểm tra trước khi gửi."}</p>
      </header>

      <form onSubmit={submit} className="grid gap-5 border border-[#16311c]/12 bg-[#eae1d2] p-6 sm:grid-cols-2 sm:p-9">
        {selectedStayName && <div className="border-l-2 border-[#80613f] bg-white/55 px-4 py-3 text-sm leading-6 sm:col-span-2">
