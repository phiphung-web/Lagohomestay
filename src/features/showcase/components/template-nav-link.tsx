...
export function TemplateNavLink({ href, label, mood, exact = false }: { href: string; label: string; mood: TemplateMood; exact?: boolean }) {
  ...
  const accent = mood === "cinematic" ? "bg-[#e5c59c]" : mood === "organic" ? "bg-[#f18b68]" : "bg-[#a36349]";
  return <Link href={href} aria-current={active ? "page" : undefined} className={`focus-ring group relative py-3 transition ${active ? "opacity-100" : "opacity-55 hover:opacity-100"}`}>
