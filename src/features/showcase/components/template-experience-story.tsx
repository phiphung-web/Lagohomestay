...
export function TemplateExperienceStory({ mood, locale = "vi" }: { mood: Mood; locale?: ShowcaseLocale }) {
  ...
  if (mood === "organic") {
    ...
    return <>
      <section className="mx-auto w-[min(1380px,calc(100%-28px))] py-20 sm:py-28"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-12">{experienceMoments.map(({ icon: Icon, time, title, text }, index) => <article key={time} className={`group relative min-h-[310px] overflow-hidden rounded-[38px] p-7 shadow-[0_20px_60px_rgba(33,72,61,.08)] transition duration-500 hover:-translate-y-2 ${cardStyles[index]} ${index === 2 ? "text-[#17321d]" : "text-[#17321d]"}`}><span aria-hidden="true" className="absolute -right-12 -top-12 h-40 w-40 rounded-full border-[24px] border-[#17321d]/5 transition duration-500 group-hover:scale-110" /><div className="relative flex h-full flex-col"><div className="flex items-center justify-between"><span className="rounded-full bg-[#17321d] px-4 py-2 text-[.62rem] font-extrabold text-white">{time}</span><Icon className="h-7 w-7" /></div><div className="mt-auto pt-16"><p className="text-[.58rem] font-extrabold uppercase tracking-[.14em] opacity-80">Khoảnh khắc 0{index + 1}</p><h2 className="mt-3 text-3xl font-extrabold tracking-[-.04em] sm:text-4xl">{title}</h2><p className="mt-3 max-w-xl text-sm font-medium leading-6 opacity-80">{text}</p></div></div></article>)}</div></section>
      <section className="mx-auto mb-24 w-[min(1280px,calc(100%-28px))]"><div className="mb-8 flex items-end justify-between"><h2 className="text-4xl font-extrabold tracking-[-.04em] sm:text-5xl">LAKA chuẩn bị gì?</h2><ArrowDownRight className="h-8 w-8 text-[#e66e4c]" /></div><div className="grid gap-4 md:grid-cols-3">{stayPromises.map(({ icon: Icon, title, text }) => <article key={title} className="rounded-[30px] border border-[#17321d]/8 bg-white p-7 shadow-[0_16px_45px_rgba(33,72,61,.06)]"><span className="grid h-12 w-12 place-items-center rounded-full bg-[#dce9c6]"><Icon className="h-6 w-6 text-[#e66e4c]" /></span><h3 className="mt-7 text-xl font-extrabold">{title}</h3><p className="mt-3 text-sm leading-6 text-[#17321d]/78">{text}</p></article>)}</div></section>
    </>;
  }
