...
export function CinematicStoryModal({ frames }: { frames: StoryFrame[] }) {
  ...
  return <>
    <button ref={triggerRef} type="button" onClick={() => { setActive(0); setPlaying(true); setOpen(true); }} className="group flex items-center gap-4 text-left">
      <span className="grid h-16 w-16 place-items-center rounded-full border border-white/30 bg-white/10 backdrop-blur transition duration-500 group-hover:scale-105 group-hover:border-[#c7a882] group-hover:bg-[#c7a882] group-hover:text-[#0b190f]"><CirclePlay className="h-6 w-6" /></span>
      <span><strong className="block text-sm">Xem cảm xúc LAKA</strong><small className="mt-1 block text-white/48">04 cảnh · Thước phim ảnh</small></span>
    </button>
    {open && typeof document !== "undefined" && createPortal(<section ref={dialogRef} role="dialog" aria-modal="true" aria-label="Thước phim ảnh về LAKA" onPointerDown={(event) => { dragStartRef.current = event.clientX; }} onPointerUp={(event) => { if (dragStartRef.current === null) return; const distance = event.clientX - dragStartRef.current; if (Math.abs(distance) > 55) change(distance < 0 ? 1 : -1); dragStartRef.current = null; }} className="fixed inset-0 z-[110] touch-pan-y overflow-hidden bg-[#020806] text-white">
      ...
      <header className="absolute inset-x-0 top-0 z-10 flex items-center justify-between p-5 sm:p-8"><span className="text-[.62rem] font-bold uppercase tracking-[.25em] text-white/55">LAKA · Thước phim thiên nhiên</span><button ref={closeRef} type="button" onClick={() => setOpen(false)} aria-label="Đóng thước phim" className="focus-ring grid h-12 w-12 place-items-center rounded-full border border-white/20 bg-black/15 backdrop-blur transition hover:bg-white hover:text-[#0b190f]"><X className="h-5 w-5" /></button></header>
      <div className="absolute inset-x-0 top-24 z-10 mx-auto flex w-[min(1100px,calc(100%-40px))] gap-2">{frames.map((item, index) => <button key={item.title} type="button" onClick={() => setActive(index)} aria-label={`Xem cảnh ${index + 1}: ${item.title}`} className="group h-8 flex-1 py-3"><span className={`block h-px transition ${index === active ? "bg-[#c7a882]" : "bg-white/25 group-hover:bg-white/55"}`} /></button>)}</div>
      <div className="absolute inset-x-0 bottom-0 z-10 mx-auto grid w-[min(1400px,calc(100%-40px))] gap-8 pb-24 sm:pb-14 lg:grid-cols-[1fr_320px] lg:items-end"><div><p className="text-[.62rem] font-bold uppercase tracking-[.24em] text-[#c7a882]">{frame.eyebrow} · 0{active + 1}</p><h2 className="mt-5 max-w-4xl font-serif text-5xl font-medium leading-[.92] tracking-[-.05em] sm:text-7xl lg:text-8xl">{frame.title}</h2></div><div className="lg:pb-2"><p className="text-sm leading-7 text-white/58">{frame.text}</p><div className="mt-7 flex items-center gap-3"><button type="button" onClick={() => change(-1)} aria-label="Cảnh trước" className="focus-ring grid h-12 w-12 place-items-center rounded-full border border-white/20 transition hover:border-white/55"><ChevronLeft className="h-5 w-5" /></button><button type="button" onClick={() => setPlaying((value) => !value)} aria-label={playing ? "Tạm dừng" : "Tiếp tục phát"} className="focus-ring grid h-12 w-12 place-items-center rounded-full bg-[#c7a882] text-[#0b190f]">{playing ? <CirclePause className="h-5 w-5" /> : <CirclePlay className="h-5 w-5" />}</button><button type="button" onClick={() => change(1)} aria-label="Cảnh tiếp theo" className="focus-ring grid h-12 w-12 place-items-center rounded-full border border-white/20 transition hover:border-white/55"><ChevronRight className="h-5 w-5" /></button></div></div></div>
