    ...
    <section className="mx-auto w-[min(1280px,calc(100%-40px))] py-20 sm:py-28">
      <div className="grid gap-10 lg:grid-cols-[.4fr_1fr]">
        ...
        <div className="border-t border-[#16311c]/15">
          {diningStories.map((item, index) => { const Icon = item.icon; return <article key={item.title.vi} className="group grid gap-6 border-b border-[#16311c]/15 py-7 sm:grid-cols-[180px_1fr_.7fr] sm:items-center">
            <div className="relative aspect-[4/3] overflow-hidden"><Image src={item.image} alt={`${inLocale(item.title, locale)} — ${locale === "en" ? "concept image" : "hình ảnh minh họa"}`} fill sizes="(max-width:640px) 100vw, 180px" className="object-cover transition duration-700 group-hover:scale-[1.04]" /><span className="absolute left-3 top-3 bg-[#16311c]/72 px-2.5 py-1 text-[.52rem] font-bold uppercase tracking-wider text-white backdrop-blur">0{index + 1}</span></div>
            <div><Icon className="h-5 w-5 text-[#80613f]" /><p className="mt-5 text-[.58rem] font-bold uppercase tracking-[.16em] text-[#16311c]/48">{inLocale(item.kicker, locale)}</p><h3 className="mt-2 font-serif text-3xl font-medium">{inLocale(item.title, locale)}</h3></div>
            <p className="text-sm leading-7 text-[#16311c]/65">{inLocale(item.text, locale)}</p>
          </article>; })}
        </div>
      </div>
    </section>
