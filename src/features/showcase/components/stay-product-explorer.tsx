...
import { getUnitsForStay, stays, stayZones, type Stay } from "@/features/stays/data/stay-catalog";
...
const sharedPolicies = {
  vi: [
    [
      "Nhận và trả căn",
      "Khung giờ chính thức nhận phòng từ 14:00 và trả phòng trước 12:00. LAKA xác nhận cụ thể cùng thông tin căn và lịch trống trước chuyến đi.",
    ],
    [
      "Số khách",
      "Số khách không vượt quá sức chứa công bố của từng dòng căn. Trẻ em dưới 6 tuổi được miễn phí theo tiêu chuẩn.",
    ],
    [
      "Không gian yên tĩnh",
      "LAKA hướng đến kỳ nghỉ yên bình, giữ âm lượng vừa phải sau 22:00 và trao đổi trước với ban quản lý nếu có hoạt động nhóm.",
    ],
    [
      "Thông tin vận hành",
      "Các dịch vụ tiện ích bao gồm bể bơi bốn mùa, xe đạp, bi-a và boardgame đã bao gồm trọn gói trong giá phòng.",
    ],
  ],
  en: [
    [
      "Arrival and departure",
      "Standard check-in from 14:00 and check-out before 12:00. LAKA confirms details with the unit and availability before the stay.",
    ],
    [
      "Guest numbers",
      "Guest numbers may not exceed the stated capacity. Children under 6 stay free of charge.",
    ],
    [
      "A quiet setting",
      "Please keep noise considerate after 10 pm and discuss group activities with LAKA in advance.",
    ],
    [
      "Operating information",
      "Amenities including four-season swimming pool, bicycles, billiards, and board games are fully included.",
    ],
  ],
} as const;

function localizedUnit(unit: ReturnType<typeof getUnitsForStay>[number], locale: ShowcaseLocale) {
  return {
    ...unit,
...
export function StayProductExplorer({
  ...
  const localizedStays = useMemo(() => stays.map((stay) => localizeStay(stay, locale)), [locale]);

  const zoneParam = searchParams.get("khu");
  const activeZoneSlug = localizedZones.some((zone) => zone.slug === zoneParam) ? zoneParam : null;
  ...
  const activeGalleryIsConcept = gallery[galleryIndex]
    ? isConceptImage(gallery[galleryIndex])
    : false;
  ...
  // =========================================================================
  const modal =
    mounted && activeStay && selectedZone
      ? createPortal(
          <div
            className="fixed inset-0 z-[100] bg-[#07140f]/80 p-0 backdrop-blur-md lg:p-4"
            role="presentation"
          >
            <button
              type="button"
            ...
            >
                ...
                <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-4 text-white sm:bottom-7 sm:left-7 sm:right-7">
                  <div>
                    <p className="text-[.6rem] font-bold uppercase tracking-[.2em] text-[#dfc6a5]">
                      {selectedZone.name}
                      {activeGalleryIsConcept
                        ? ` · ${locale === "en" ? "Concept image" : "Hình ảnh minh họa"}`
                        : ""}
                    </p>
                    <p className="mt-1.5 font-serif text-2xl sm:text-4xl font-semibold tracking-tight">
                      {activeStay.name}
                    </p>
                  </div>
              ...
              <section className="relative flex min-h-full min-w-0 flex-col overflow-x-hidden lg:h-full lg:overflow-y-auto">
                <div className="px-5 pb-5 pt-6 sm:px-9 sm:pb-6 sm:pt-8 lg:px-11 lg:pr-20">
                  ...
                  <h2
                    id="stay-dialog-title"
                    className="font-serif text-3xl sm:text-4xl font-semibold tracking-tight text-[#16311c] mt-3"
                  >
                    {activeStay.name}
                  </h2>
                  <p className="mt-2 text-sm text-[#16311c]/70 font-serif italic">
                    {activeStay.subtitle}
                  </p>
                  ...
                  <div className="mt-6 flex flex-wrap gap-x-5 gap-y-2 border-y border-[#16311c]/12 py-4 text-xs font-bold text-[#16311c]/85">
                    {activeStay.maxGuests > 0 && (
                      <span className="flex items-center gap-1.5">
                        <Users className="h-4 w-4 text-[#80613f]" />
                        Tối đa {activeStay.maxGuests} khách
                      </span>
                    )}
                    {activeStay.beds > 0 && (
                      <span className="flex items-center gap-1.5">
                        <BedDouble className="h-4 w-4 text-[#80613f]" />
                        {activeStay.beds} giường
                      </span>
                    )}
                    {activeStay.area > 0 && (
                      <span className="flex items-center gap-1.5">
                        <House className="h-4 w-4 text-[#80613f]" />
                        {activeStay.area} m²
                      </span>
                    )}
                    <span className="flex items-center gap-1.5">
                      <Bath className="h-4 w-4 text-[#80613f]" />
                      WC khép kín
                    </span>
                  </div>
                </div>
                ...
                <nav
                  aria-label={locale === "en" ? "Home details" : "Nội dung chi tiết căn"}
                  className="z-[2] flex shrink-0 overflow-x-auto no-scrollbar border-y border-[#16311c]/12 bg-[#eae1d2] px-5 sm:px-9 lg:px-11"
                >
                  {(
                    [
                      ...
                      [
                        "amenities",
                        locale === "en" ? "Amenities & Inclusions" : "Tiện nghi & Dịch vụ",
                      ],
                    ...
                    >
                      {label}
                      {detailSection === key && (
                        <span className="absolute inset-x-3 bottom-0 h-0.5 bg-[#80613f] first:left-0" />
                      )}
                    </button>
                  ))}
                </nav>
                ...
                <div className="flex-1 px-5 py-7 sm:px-9 lg:px-11">
                  {detailSection === "overview" && (
                    <div>
                      <p className="text-sm sm:text-base leading-relaxed text-[#16311c]/85">
                        {activeStay.longDescription}
                      </p>
                      ...
                      <div className="mt-8">
                        <p className="text-[.6rem] font-bold uppercase tracking-[.18em] text-[#80613f]">
                          {locale === "en" ? "Best for" : "Phù hợp nhất"}
                        </p>
                        <div className="mt-3 flex flex-wrap gap-2">
                          {activeStay.idealFor.map((item) => (
                            <span
                              key={item}
                              className="rounded-full border border-[#16311c]/15 bg-white/70 px-4 py-1.5 text-xs font-bold text-[#16311c]"
                            >
                              {item}
                            </span>
                          ))}
                      ...
                      {selectedUnits.length > 0 && (
                        <div className="mt-8">
                          <p className="text-[.6rem] font-bold uppercase tracking-[.18em] text-[#80613f]">
                            {locale === "en" ? "Physical homes" : "Danh sách căn thuộc dòng này"} (
                            {selectedUnits.length} căn)
                          </p>
                          <div className="mt-3 grid gap-2 sm:grid-cols-2">
                            {selectedUnits.map((unit) => (
                              <article
                                key={unit.id}
                                className="rounded-xl border border-[#16311c]/10 bg-white/60 p-3.5"
                              >
                                <div className="flex items-center justify-between gap-3">
                                  <span className="text-[.6rem] font-extrabold uppercase tracking-wider text-[#80613f]">
                                    {unit.code}
                                  </span>
                                  <MapPin className="h-3.5 w-3.5 opacity-40 text-[#16311c]" />
                                </div>
                                <h4 className="font-bold text-sm text-[#16311c] mt-1">
                                  {unit.name}
                                </h4>
                    ...
                    <div>
                      ...
                      <div className="mt-4 space-y-3 rounded-xl border border-[#16311c]/10 bg-white/60 p-5">
                        {activeStay.included.map((item) => (
                          <p
                            key={item}
                            className="flex gap-3 text-xs sm:text-sm leading-relaxed text-[#16311c]"
                          >
                      ...
                      <div className="mt-3 grid gap-2.5 sm:grid-cols-2">
                        {activeStay.amenities.map((item) => (
                          <span
                            key={item}
                            className="flex items-center gap-2.5 rounded-lg border border-[#16311c]/8 bg-white/40 px-3.5 py-2.5 text-xs font-medium text-[#16311c]"
                          >
                      ...
                      <div className="mt-3 divide-y divide-[#16311c]/10 border-y border-[#16311c]/10">
                        {policies.map(([title, text], index) => (
                          <details key={title} open={index === 0} className="group py-2">
                            ...
                            <p className="pb-4 pr-6 text-xs sm:text-sm leading-relaxed text-[#16311c]/70">
                              {text}
                            </p>
                          </details>
                        ))}
                ...
                <div className="sticky bottom-0 z-[3] mt-auto border-t border-[#16311c]/12 bg-[#eae1d2]/98 px-5 py-4 backdrop-blur sm:px-9 lg:px-11">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="text-xs text-[#80613f] font-medium">
                        {selectedZone.name} · {activeStay.badge}
                      </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2.5">
                      ...
                      >
                        <span>
                          {locale === "en" ? "Contact about this stay" : "Liên hệ về căn này"}
                        </span>
  ...
  const inquiryModal =
    mounted && inquiryStay
      ? createPortal(
          <div
            className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-[#07140f]/85 backdrop-blur-md"
            role="presentation"
          >
            <button
              type="button"
              ...
              <div className="flex items-center justify-between border-b border-[#16311c]/12 bg-[#ded2c0] px-6 py-4">
                <div>
                  ...
                  <h3
                    ...
                    className="font-serif text-xl sm:text-2xl font-bold text-[#16311c]"
                  >
                    {inquiryStay.name}
                  </h3>
                </div>
  ...
  return (
    <>
      <section
        id="bo-suu-tap-can"
        className="scroll-mt-[92px] bg-[#eae1d2] py-10 sm:py-14 text-[#16311c]"
      >
                    ...
                    <span
                      ...
                      className={`absolute inset-x-0 bottom-0 h-0.5 bg-[#80613f] origin-left transition-transform duration-300 ${
                        selected
                          ...
                          : "scale-x-0 group-hover:scale-x-100 opacity-60"
                      }`}
                    />
                  </button>
                );
            ...
            {visibleStays.map((stay) => {
              ...
              return (
                <article
                  ...
                  aria-label={
                    locale === "en" ? `View ${stay.name} details` : `Xem chi tiết ${stay.name}`
                  }
                >
                  ...
                  <div className="absolute top-4 left-4 right-4 z-10 flex items-center justify-between gap-2">
                    ...
                    <span className="rounded-full bg-white/20 px-2.5 py-1 text-[.6rem] font-bold text-white backdrop-blur-md border border-white/10">
                      {stay.area > 0 ? `${stay.area}m² · ` : ""}
                      {stay.maxGuests > 0 ? `Tối đa ${stay.maxGuests} khách` : "Riêng tư"}
                    </span>
                  </div>
              ...
              <div className="divide-y divide-[#16311c]/12 border-y border-[#16311c]/12">
                {policies.map(([title, text], index) => (
                  <details key={title} open={index === 0} className="group">
                    ...
                    <p className="max-w-2xl pb-5 pr-8 text-xs sm:text-sm leading-relaxed text-[#16311c]/65">
                      {text}
                    </p>
                  </details>
                ))}
