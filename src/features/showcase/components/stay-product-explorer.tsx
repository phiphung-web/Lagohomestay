...
export function StayProductExplorer({
  ...
  // Clean, evenly-spaced tabs matching the 3 zones without counts
  const tabs = [
    ...
    <>
      <section id="bo-suu-tap-can" className="scroll-mt-[92px] bg-[#eae1d2] py-10 sm:py-14 text-[#16311c]">
        <div className="mx-auto w-[min(1460px,calc(100%-28px))] sm:w-[min(1460px,calc(100%-48px))]">
          {/* ================================================================= */}
          {/* 1. BALANCED TAB MENU (4 Columns evenly spaced, no count numbers) */}
          {/* ================================================================= */}
          <nav
            aria-label={locale === "en" ? "Filter stays by zone" : "Lọc căn theo khu lưu trú"}
            className="sticky top-[var(--laka-header-offset)] z-20 overflow-x-auto rounded-2xl border border-[#16311c]/12 bg-[#eae1d2]/95 p-1.5 sm:p-2 backdrop-blur-md shadow-sm transition-[top] duration-300"
          >
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`focus-ring flex items-center justify-center rounded-xl py-3 sm:py-3.5 px-3 text-center text-xs sm:text-sm font-bold tracking-wide transition duration-200 ${
                      selected
                        ? "bg-[#16311c] text-[#eae1d2] shadow-md scale-[1.01]"
                        : "text-[#16311c]/70 hover:bg-black/5 hover:text-[#16311c]"
                    }`}
                  >
                    <span>{tab.label}</span>
                  </button>
