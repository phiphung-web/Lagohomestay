        ...
        <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {faqCategories[locale].map(([label, target], index) => (
            <a key={label} href={`#faq-${target}`} className="focus-ring group flex min-h-32 items-end justify-between border border-[#80613f]/45 bg-[#eae1d2] p-5 transition hover:-translate-y-1 hover:bg-[#f2ece2]">
              ...
              <span className="laka-heading-card mt-5 block">{label}</span>
              </span>
...
export function TemplateFaqSection({ mood, policyHref, locale = "vi" }: { mood: Mood; policyHref: string; locale?: ShowcaseLocale }) {
  ...
  if (mood === "organic") return <section className="mx-auto w-[min(1280px,calc(100%-28px))] py-20 sm:py-28">
    <div className="mb-9 flex flex-col gap-6 rounded-[34px] bg-[#16311c] p-7 text-white sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[.6rem] font-extrabold uppercase tracking-[.14em] text-[#f7cf58]">Bạn cứ hỏi nhé</p><p className="mt-3 text-2xl font-extrabold">LAKA trả lời rõ trước khi bạn lên đường.</p></div><a href={publicContact.phoneHref} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#f7cf58] px-6 text-sm font-extrabold text-[#16311c]"><Phone className="h-4 w-4" />Gọi LAKA</a></div>
  ...
  return <section className="mx-auto grid w-[min(1120px,calc(100%-40px))] gap-12 py-20 sm:py-28 lg:grid-cols-[.55fr_1fr]">
    <aside className="h-fit lg:sticky lg:top-28"><p className="text-[.62rem] font-bold uppercase tracking-[.18em] text-[#80613f]">{locale === "en" ? "Need more information?" : "Cần thêm thông tin?"}</p><p className="laka-section-lead mt-6 max-w-sm">{locale === "en" ? "We are ready to listen before you decide." : "Chúng mình sẵn sàng lắng nghe trước khi bạn quyết định."}</p><p className="mt-4 max-w-sm text-sm leading-7 text-[#16311c]/72">{locale === "en" ? "Call or message us on Zalo and the LAKA team will advise you based on your group." : "Gọi hoặc nhắn Zalo, đội ngũ LAKA sẽ tư vấn theo nhu cầu của từng đoàn."}</p><a href={publicContact.phoneHref} className="mt-7 inline-flex items-center gap-2 border-b border-[#16311c] pb-1 text-sm font-bold"><Phone className="h-4 w-4" />{publicContact.phoneDisplay}</a></aside>
    <div className="border-t border-[#16311c]/15">{localizedFaqs.map(([question, answer], index) => <details id={`faq-${index + 1}`} key={question} className="group scroll-mt-28 border-b border-[#16311c]/15 py-7"><summary className="flex cursor-pointer list-none items-start gap-5"><span className="pt-1 text-[.6rem] font-bold text-[#80613f]">0{index + 1}</span><span className="laka-heading-card flex-1">{question}</span><Plus className="mt-1 h-5 w-5 shrink-0 transition duration-300 group-open:rotate-45" /></summary><div className="pl-10"><Answer text={answer} /></div></details>)}<Link href={policyHref} className="mt-8 inline-flex items-center gap-2 text-sm font-bold">{locale === "en" ? "Read stay policies" : "Đọc chính sách lưu trú"} <ArrowRight className="h-4 w-4" /></Link></div>
  </section>;
}
