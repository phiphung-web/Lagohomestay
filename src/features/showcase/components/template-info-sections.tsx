...
export function TemplateFaqSection({ mood, policyHref, locale = "vi" }: { mood: Mood; policyHref: string; locale?: ShowcaseLocale }) {
  ...
  return <section className="mx-auto grid w-[min(1120px,calc(100%-40px))] gap-12 py-20 sm:py-28 lg:grid-cols-[.55fr_1fr]">
    <aside className="h-fit lg:sticky lg:top-28"><p className="text-[.62rem] font-bold uppercase tracking-[.18em] text-[#80613f]">{locale === "en" ? "Need more information?" : "Cần thêm thông tin?"}</p><p className="mt-6 max-w-sm font-serif text-3xl font-medium leading-tight">{locale === "en" ? "We are ready to listen before you decide." : "Chúng mình sẵn sàng lắng nghe trước khi bạn quyết định."}</p><p className="mt-4 max-w-sm text-sm leading-7 text-[#16311c]/72">{locale === "en" ? "Call or message us on Zalo and the LAKA team will advise you based on your group." : "Gọi hoặc nhắn Zalo, đội ngũ LAKA sẽ tư vấn theo nhu cầu của từng đoàn."}</p><a href={publicContact.phoneHref} className="mt-7 inline-flex items-center gap-2 border-b border-[#16311c] pb-1 text-sm font-bold"><Phone className="h-4 w-4" />{publicContact.phoneDisplay}</a></aside>
