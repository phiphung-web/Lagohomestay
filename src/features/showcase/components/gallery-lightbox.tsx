...
export function GalleryLightbox({ images, mood, locale = "vi" }: { images: string[]; mood: TemplateMood; locale?: ShowcaseLocale }) {
  ...
  return <>
    <section className={`gallery-grid gallery-grid-${mood} mx-auto columns-1 gap-4 py-16 sm:w-[min(1420px,calc(100%-32px))] sm:columns-2 lg:columns-3`}>
      {images.map((src, index) => <button type="button" onClick={(event) => { returnFocusRef.current = event.currentTarget; setActive(index); }} key={`${src}-${index}`} aria-label={locale === "en" ? `Open concept image ${index + 1}` : `Mở ảnh minh họa ${index + 1}`} className={`group focus-ring relative mb-4 block w-full break-inside-avoid overflow-hidden text-left ${index % 4 === 0 ? "aspect-[3/4]" : "aspect-[4/3]"}`}><Image src={src} alt={locale === "en" ? `LAKA Homestay — concept image ${index + 1}` : `LAKA Homestay — ảnh minh họa ${index + 1}`} fill sizes="(max-width:768px) 100vw, 33vw" className="object-cover transition duration-700 group-hover:scale-[1.035]" /><span className="absolute inset-0 bg-black/0 transition group-hover:bg-black/15" /><span className="absolute right-3 top-3 grid h-10 w-10 translate-y-2 place-items-center rounded-full bg-white text-[#17321d] opacity-0 shadow-xl transition group-hover:translate-y-0 group-hover:opacity-100"><Expand className="h-4 w-4" /></span><span className="absolute bottom-3 left-3 rounded-full bg-black/52 px-3 py-1.5 text-[.58rem] font-bold uppercase tracking-wider text-white backdrop-blur">{locale === "en" ? "Concept" : "Minh họa"} · {String(index + 1).padStart(2, "0")}</span></button>)}
    </section>
