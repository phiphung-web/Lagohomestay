...
export function TemplateMobileMenu({ name, mood, items, bookingHref, lookupHref, locale = "vi", wideHeader = false }: { name: string; mood: TemplateMood; items: MenuItem[]; bookingHref: string; lookupHref: string; locale?: ShowcaseLocale; wideHeader?: boolean }) {
  ...
  return <>
    ...
    {open && typeof document !== "undefined" && createPortal(<div className="fixed inset-0 z-[105] bg-[#06120e]/58 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
      <section ref={panelRef} role="dialog" aria-modal="true" aria-label={locale === "en" ? `${name} website menu` : `Menu website ${name}`} className={`template-menu-enter absolute overflow-y-auto overscroll-contain p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl ${cinematic ? "inset-0 bg-[#0b190f] text-white sm:left-auto sm:w-[430px]" : organic ? "inset-x-2 bottom-2 max-h-[calc(100svh-16px)] rounded-[34px] bg-[#eef3df] text-[#16311c] sm:left-auto sm:right-4 sm:w-[430px]" : "bottom-0 right-0 top-0 w-[min(430px,100%)] bg-[#eae1d2] text-[#16311c]"}`}>
        ...
        <div className="mt-7 grid gap-2 border-t border-current/12 pt-5">
          <Link href={bookingHref} onClick={close} className={`focus-ring flex min-h-12 items-center justify-between rounded-full px-5 text-sm font-bold ${cinematic ? "bg-[#c7a882] text-[#0b190f]" : "bg-[#16311c] text-white"}`}>
            {locale === "en" ? "Check availability" : "Xem lịch trống"}<ArrowRight className="h-4 w-4" />
          </Link>
