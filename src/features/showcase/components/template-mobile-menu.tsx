...
export function TemplateMobileMenu({ name, mood, items, bookingHref, lookupHref, contactHref, locale = "vi" }: { name: string; mood: TemplateMood; items: MenuItem[]; bookingHref: string; lookupHref: string; contactHref: string; locale?: ShowcaseLocale }) {
  ...
  return <>
    <button ref={triggerRef} type="button" onClick={() => setOpen(true)} aria-haspopup="dialog" aria-expanded={open} aria-label={locale === "en" ? "Open menu" : "Mở menu"} className="focus-ring grid h-11 w-11 place-items-center rounded-full border border-current/15 lg:hidden"><Menu className="h-5 w-5" /></button>
    {open && typeof document !== "undefined" && createPortal(<div className="fixed inset-0 z-[105] bg-[#06120e]/58 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
      <section ref={panelRef} role="dialog" aria-modal="true" aria-label={locale === "en" ? `${name} website menu` : `Menu website mẫu ${name}`} className={`template-menu-enter absolute overflow-y-auto overscroll-contain p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl ${cinematic ? "inset-0 bg-[#0b190f] text-white sm:left-auto sm:w-[430px]" : organic ? "inset-x-2 bottom-2 max-h-[calc(100svh-16px)] rounded-[34px] bg-[#eef3df] text-[#17321d] sm:left-auto sm:right-4 sm:w-[430px]" : "bottom-0 right-0 top-0 w-[min(430px,100%)] bg-[#faf3ea] text-[#17321d]"}`}>
