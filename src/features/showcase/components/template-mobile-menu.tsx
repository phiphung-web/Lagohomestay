...
export function TemplateMobileMenu({ name, mood, items, bookingHref, lookupHref, contactHref }: { name: string; mood: TemplateMood; items: MenuItem[]; bookingHref: string; lookupHref: string; contactHref: string }) {
  ...
  return <>
    ...
    {open && typeof document !== "undefined" && createPortal(<div className="fixed inset-0 z-[105] bg-[#06120e]/58 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
      <section ref={panelRef} role="dialog" aria-modal="true" aria-label={`Menu website mẫu ${name}`} className={`template-menu-enter absolute overflow-y-auto overscroll-contain p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl ${cinematic ? "inset-0 bg-[#0b190f] text-white sm:left-auto sm:w-[430px]" : organic ? "inset-x-2 bottom-2 max-h-[calc(100svh-16px)] rounded-[34px] bg-[#eef3df] text-[#17321d] sm:left-auto sm:right-4 sm:w-[430px]" : "bottom-0 right-0 top-0 w-[min(430px,100%)] bg-[#faf3ea] text-[#17321d]"}`}>
        ...
        <div className="mt-7 grid gap-3"><Link href={bookingHref} onClick={close} className={`flex min-h-14 items-center justify-center gap-2 rounded-full px-5 text-sm font-bold ${cinematic ? "bg-[#c7a882] text-[#0b190f]" : organic ? "bg-[#f18b68]" : "bg-[#17321d] text-white"}`}><CalendarDays className="h-4 w-4" />Kiểm tra lịch trống</Link><div className="grid grid-cols-2 gap-3"><Link href={lookupHref} onClick={close} className="flex min-h-12 items-center justify-center gap-2 rounded-full border border-current/15 text-xs font-bold"><Search className="h-4 w-4" />Tra cứu</Link><Link href={contactHref} onClick={close} className="flex min-h-12 items-center justify-center gap-2 rounded-full border border-current/15 text-xs font-bold"><Phone className="h-4 w-4" />Liên hệ</Link></div></div>
      </section>
    </div>, document.body)}
  </>;
}
