...
export function TemplateStaysCollection({ mood, basePath }: { mood: Mood; basePath: string }) {
  if (mood === "cinematic") return <section className="mx-auto w-[min(1500px,calc(100%-40px))] py-20 sm:py-28">
    ...
    <div>{stays.map((stay, index) => <Link href={`${basePath}/luu-tru/${stay.slug}`} key={stay.id} className="group grid gap-5 border-b border-white/12 py-7 sm:grid-cols-[52px_1fr] lg:grid-cols-[64px_minmax(260px,.8fr)_minmax(300px,1fr)_190px] lg:items-center">
      ...
      <div className="flex items-end justify-between gap-4 lg:block lg:text-right"><StayFacts stay={stay} compact /><p className="mt-4 text-sm font-bold text-[#c7a882]">Từ {formatCurrency(stay.basePrice)}</p><ArrowRight className="mt-5 ml-auto hidden h-5 w-5 transition group-hover:translate-x-1 lg:block" /></div>
    </Link>)}</div>
  </section>;
