...
export function TemplateStayHero({ mood, basePath, stay, locale = "vi" }: { mood: Mood; basePath: string; stay: Stay; locale?: ShowcaseLocale }) {
  ...
  if (mood === "editorial") return <section className="relative min-h-[100svh] overflow-hidden bg-[#10251d] text-white">
    ...
    <div className="relative z-10 mx-auto flex min-h-[100svh] w-[min(1480px,calc(100%-40px))] flex-col justify-between pb-12 pt-28 sm:pb-16">
      ...
      <div><p className="laka-eyebrow text-[#dfc6a5]">{zone.name} · {stay.subtitle}</p><h1 className="laka-display-hero mt-5">{stay.name}</h1><div className="mt-7 grid gap-7 border-t border-white/24 pt-6 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="laka-body max-w-xl text-white/66">{stay.description}</p><div className="mt-5 flex flex-wrap items-center gap-4"><StayFacts stay={stay} locale={locale} /><span className="text-xs font-bold text-[#dfc6a5]">{units.length} {locale === "en" ? "physical homes" : "căn thực tế"} · {units.map((unit) => unit.code).join(" / ")}</span></div></div><a href="#khong-gian" className="inline-flex min-h-14 items-center gap-3 rounded-full bg-[#eae1d2] px-6 text-sm font-bold text-[#16311c]">{locale === "en" ? "Enter the space" : "Đi vào không gian"} <ArrowRight className="h-4 w-4" /></a></div></div>
    </div>
  </section>;

  if (mood === "organic") return <section className="min-h-[100svh] overflow-hidden px-3 py-4 sm:px-5 sm:py-7">
  ...
  return <section className="grain relative min-h-[100svh] overflow-hidden"><Image src={stay.image} alt={`${stay.name} - ảnh minh họa`} fill priority sizes="100vw" className="object-cover opacity-72 transition duration-[1400ms] hover:scale-[1.015]" /><div className="absolute inset-0 bg-gradient-to-t from-black/88 via-black/18 to-black/35" /><div className="relative z-10 mx-auto flex min-h-[100svh] w-[min(1420px,calc(100%-40px))] flex-col justify-end pb-12 text-white sm:pb-16"><Link href={backHref} className="mb-10 text-[.62rem] font-bold uppercase tracking-[.2em] text-white/58">← Film index / Bộ sưu tập</Link><p className="text-[.62rem] font-bold uppercase tracking-[.24em] text-[#c7a882]">Chương riêng · {stay.subtitle}</p><h1 className="mt-5 font-serif text-[clamp(4rem,13vw,9rem)] font-medium leading-[.84] tracking-[-.065em]">{stay.name}</h1><div className="mt-8 flex flex-wrap items-end justify-between gap-7 border-t border-white/18 pt-6"><StayFacts stay={stay} /><a href="#khong-gian" className="inline-flex min-h-12 items-center gap-2 text-sm font-bold text-[#c7a882]">Đi vào không gian <ArrowRight className="h-4 w-4" /></a></div></div>
  </section>;
}
