    ...
    return (
      <>
        <label htmlFor={pickerId} className="sr-only">
          {en ? "Choose a menu category" : "Chọn nhóm món"}
        </label>
        <select
          id={pickerId}
          value={active}
          onChange={(event) => selectPage(Number(event.target.value))}
          className={`focus-ring ${styles.select}`}
        >
          {restaurantMenuPages.map((item, index) => (
    ...
    return (
      <div className={styles.pagination}>
        <button
          ...
          aria-label={en ? "Previous menu page" : "Trang menu trước"}
        ...
        <span aria-live="polite" aria-atomic="true" className={styles.pageCount}>
          ...
          {pageNumber} <span className="opacity-40">/ {count}</span>
        </span>
        <button
          type="button"
          onClick={() => selectPage(active + 1)}
          aria-label={en ? "Next menu page" : "Trang menu tiếp theo"}
          className={`focus-ring ${styles.iconButton}`}
        >
      ...
      <header className="mb-7 flex flex-wrap items-end justify-between gap-5">
        <div>
          <p className="laka-eyebrow text-[#80613f]">
            {en ? "Lakeside Restaurant" : "Nhà Hàng Ven Hồ"}
          </p>
          <h2 id={`${id}-heading`} className="laka-heading-section mt-3">
            {en ? "Restaurant menu" : "Thực đơn nhà hàng"}
          </h2>
        </div>
        <p className="max-w-sm text-sm leading-6 text-[#16311c]/65">
          {en
            ? "Choose a category and enlarge the menu to explore each dish. Menu images are in Vietnamese."
            : "Chọn nhóm món, mở ảnh để xem rõ món ăn và giá."}
        </p>
      </header>

      <div className={styles.browser}>
        <nav
          aria-label={en ? "Restaurant menu categories" : "Các nhóm món nhà hàng"}
          className={styles.categories}
        >
          <p className="laka-eyebrow mb-4 px-3 text-[#80613f]">
            {en ? "Explore the menu" : "Chọn món theo nhóm"}
          </p>
          {restaurantMenuPages.map((item, index) => (
            <button
              key={item.id}
          ...
          <p className="mt-5 px-3 text-xs leading-6 text-[#16311c]/60">
            {en
              ? "All 10 pages belong to the restaurant, including drinks."
              : "10 trang thực đơn nhà hàng, bao gồm cả đồ uống."}
          </p>
        </nav>
        ...
        <div id={`${id}-preview`} className={styles.viewer}>
          <div className={styles.toolbar}>
            ...
            <button
              ...
              className={`focus-ring ${styles.textButton}`}
            >
          ...
          <div className={styles.toolbar}>
            <span className="hidden text-xs text-[#16311c]/60 sm:block">
              {en ? "Click the image for a closer look" : "Bấm vào ảnh để xem rõ hơn"}
            </span>
            {renderPagination()}
          </div>
        </div>
          ...
          <div className={styles.dialogLayout}>
            <header className={styles.dialogHeader}>
              <div className="min-w-0 flex-1">
                <p
                  id={`${id}-dialog-title`}
                  className="mb-1 text-xs font-semibold text-[#16311c]/60"
                >
                  {en ? "LAKA · Restaurant menu" : "LAKA · Thực đơn nhà hàng"}
                </p>
              ...
              <button
                ...
                aria-label={en ? "Close menu" : "Đóng thực đơn"}
                className={`focus-ring ${styles.iconButton}`}
              >
            ...
            <div
              ...
              onTouchEnd={(event) => {
                ...
                touchStartRef.current = null;
                if (zoomed || !start || event.touches.length || !event.changedTouches.length)
                  return;
                const dx = event.changedTouches[0].clientX - start.x;
                const dy = event.changedTouches[0].clientY - start.y;
                if (Math.abs(dx) > 60 && Math.abs(dx) > Math.abs(dy) * 1.5)
                  selectPage(active + (dx < 0 ? 1 : -1));
              }}
            >
              <Image
                key={page.src}
            ...
            <footer className={styles.dialogFooter}>
              ...
              <a
                ...
                className={`focus-ring ${styles.originalLink}`}
              >
                {en ? "Open image" : "Mở ảnh"}
