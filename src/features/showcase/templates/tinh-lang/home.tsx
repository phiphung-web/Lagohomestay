      ...
      <section className="laka-theme-body laka-theme-muted border-y border-[#16311c]/12 bg-[#e3d8c9] px-5 py-20 sm:px-8 sm:py-28">
        <div className="mx-auto w-[min(1320px,100%)]">
          <div className="grid gap-7 lg:grid-cols-[1fr_.45fr] lg:items-end">
            ...
            <div><p className="text-sm leading-7 text-[#16311c]/62">{en ? "A proposed dining concept built around the home: breakfast on the veranda, seasonal shared dishes and private evening tables." : "Concept ẩm thực xoay quanh căn nhà: giỏ sáng bên hiên, món theo mùa để chia sẻ và bàn tối riêng tư."}</p><Link href={`${config.basePath}/am-thuc`} className="mt-5 inline-flex items-center gap-2 border-b border-[#16311c] pb-2 text-xs font-bold uppercase tracking-[.12em]">{en ? "Explore LAKA dining" : "Khám phá ẩm thực LAKA"} <ArrowRight className="h-4 w-4" /></Link></div>
          </div>
