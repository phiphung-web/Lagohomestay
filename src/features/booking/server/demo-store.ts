...
const globalStore = globalThis as unknown as {
  lagoBookings?: Map<string, StoredBooking>;
  lagoIdempotency?: Map<string, string>;
};
...
export function createDemoBooking(
  data: Omit<StoredBooking, "code" | "holdExpiresAt" | "createdAt" | "status">,
  ...
  const booking: StoredBooking = {
    ...
    status: "HELD",
    holdExpiresAt: addHours(new Date(), 2).toISOString(),
...
export function isDemoUnitAvailable(unitId: string, checkIn: string, checkOut: string) {
  ...
  return ![...demoBookingStore.values()].some((item) => {
    ...
    const blocksInventory =
      item.status === "HELD" || item.status === "CONFIRMED" || item.status === "CHECKED_IN";
    return (
      item.unitId === unitId &&
      blocksInventory &&
...
export function clearDemoStoreForTests() {
  demoBookingStore.clear();
  demoIdempotencyStore.clear();
}
...
export function findDemoBookings(phone: string) {
  ...
  return [...demoBookingStore.values()]
    ...
    .map((booking) => {
      if (booking.status === "HELD" && new Date(booking.holdExpiresAt) <= new Date())
