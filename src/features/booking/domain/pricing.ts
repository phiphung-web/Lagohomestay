...
export type RateRule = {
  type: "WEEKDAY" | "DATE_RANGE" | "EXTRA_GUEST";
  amount: number;
  priority: number;
  weekdays?: number[];
  startDate?: Date;
...
export function calculatePrice(input: {
    ...
    const matching = rules
      .filter((rule) => {
        ...
        if (rule.type === "DATE_RANGE")
          return rule.startDate && rule.endDate && day >= rule.startDate && day <= rule.endDate;
        return false;
      })
  ...
  const extraRule = rules
    .filter((r) => r.type === "EXTRA_GUEST" && guests >= (r.minGuests ?? baseGuests + 1))
  ...
  return {
    ...
    seasonal,
    extraGuest,
    total: base + weekend + seasonal + extraGuest,
  };
}
