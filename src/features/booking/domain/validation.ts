import { z } from "zod";
import { PHONE_PATTERN } from "@/features/booking/domain/contact-validation";
...
export const createBookingSchema = z.object({
  ...
  fullName: z.string().trim().min(2).max(100), phone: z.string().regex(PHONE_PATTERN),
  email: z.string().email().optional().or(z.literal("")), note: z.string().max(500).optional(), consent: z.literal(true)
}).refine((v) => new Date(v.checkOut) > new Date(v.checkIn), { message: "Ngày trả phòng phải sau ngày nhận phòng" });

export const lookupBookingSchema = z.object({ phone: z.string().regex(PHONE_PATTERN) });
