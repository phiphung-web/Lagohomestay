...
export function LookupForm({ locale = "vi" }: { locale?: ShowcaseLocale }) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [results, setResults] = useState<Result[]>([]);
  const [phoneError, setPhoneError] = useState("");
  const [searched, setSearched] = useState(false);
  const localizedStayName = (name: string) => {
    const stay = stays.find((item) => item.name === name);
    return stay ? localizeStay(stay, locale).name : name;
  };
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (validationError) { setPhoneError(locale === "en" ? "Enter a valid phone number, for example 090 123 4567." : validationError); return; }
    setPhoneError("");
    setLoading(true);
    setSearched(false);
    setError("");
    ...
    try {
      ...
      else if (!response.ok) setError(locale === "en" ? "We cannot check right now. Please try again or call LAKA." : data.message);
      else { setResults(data.bookings); setSearched(true); }
    } catch {
      setError(locale === "en" ? "We cannot check right now. Please try again or call LAKA." : "Chưa thể kiểm tra lúc này. Vui lòng thử lại hoặc gọi LAKA.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return <div>
    <form noValidate aria-busy={loading} onSubmit={submit} className="card p-6 sm:p-8">
      <label className="block text-sm font-bold">{locale === "en" ? "Phone number used for booking" : "Số điện thoại đã đặt"}
      ...
      <p id="lookup-phone-hint" className="mt-3 text-xs leading-5 text-lago-ink/50">{locale === "en" ? "Use the same phone or Zalo number entered with your request." : "Dùng đúng số điện thoại hoặc số Zalo đã nhập khi gửi yêu cầu."}</p>
      ...
      <button disabled={loading} className="btn-primary mt-6 w-full">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}{locale === "en" ? "Find my booking" : "Xem thông tin đặt chỗ"}</button>
    </form>

    {results.length > 0 && <div className="mt-6 space-y-4" aria-live="polite">
      <div className="flex items-center justify-between px-1"><h2 className="font-bold">{locale === "en" ? "Your bookings" : "Đặt chỗ của bạn"}</h2><span className="text-xs text-lago-ink/45">{results.length} {locale === "en" ? "requests" : "yêu cầu"}</span></div>
      {results.map((result, index) => <article key={`${result.stayName}-${result.checkIn}-${index}`} className="card p-6 sm:p-7">
        <div className="flex items-start justify-between gap-4"><div><p className="eyebrow text-lago-moss">{locale === "en" ? "Selected home" : "Căn đã chọn"}</p><h3 className="display mt-1 text-2xl font-semibold">{localizedStayName(result.stayName)}</h3></div><StatusBadge status={result.status} locale={locale} /></div>
