...
export function BookingExperience() {
      ...
      try {
        ...
        if (selected) {
          ...
          if (refreshed) setSelected(refreshed);
            ...
            setSelected(null);
            setStep(1);
            setError("Căn bạn vừa chọn không còn trống trong khoảng ngày mới. Lago đã cập nhật các lựa chọn khác bên dưới.");
          }
  ...
  if (step === 3 && result && selected) {
    return <div className="container-lago py-16 sm:py-24"><div className="mx-auto max-w-2xl text-center"><span className="success-pulse mx-auto grid h-20 w-20 place-items-center rounded-full bg-lago-mist"><Check className="h-9 w-9" /></span><p className="eyebrow mt-8 text-lago-clay">Yêu cầu đã được ghi nhận</p><h1 className="display mt-3 text-5xl font-semibold">{selected.name} đang được giữ cho bạn.</h1><p className="mx-auto mt-5 max-w-lg leading-7 text-lago-ink/65">Đội ngũ Lago sẽ liên hệ qua điện thoại hoặc Zalo để xác nhận. Bạn chưa cần thanh toán ở bước này.</p><div className="card mt-9 grid gap-4 p-7 text-left text-sm sm:grid-cols-2"><div><span className="text-lago-ink/50">Căn nhà</span><p className="mt-1 font-bold">{selected.name}</p></div><div><span className="text-lago-ink/50">Tạm tính</span><p className="mt-1 font-bold">{formatCurrency(result.totalAmount)}</p></div><div><span className="text-lago-ink/50">Thời gian</span><p className="mt-1 font-bold">{format(parseISO(checkIn), "dd/MM/yyyy")} → {format(parseISO(checkOut), "dd/MM/yyyy")}</p></div><div><span className="text-lago-ink/50">Giữ đến</span><p className="mt-1 font-bold">{new Date(result.holdExpiresAt).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })}</p></div></div><p className="mt-5 text-sm text-lago-ink/55">Bạn có thể xem lại yêu cầu bất cứ lúc nào bằng số điện thoại đã đặt.</p><div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row"><a className="btn-primary" href="tel:0900000000"><Phone className="h-4 w-4" /> Gọi Lago</a><a className="btn-light border border-lago-ink/15" href="https://zalo.me/0900000000">Liên hệ Zalo</a></div><Link href="/tra-cuu" className="soft-link mt-7">Xem lại đặt chỗ <ArrowRight className="h-4 w-4" /></Link></div></div>;
  }
  ...
  return <div className="container-lago py-12 sm:py-20">
    ...
    {step === 1 && <>
      ...
      {availabilityState === "error" && <div className="mt-10 rounded-[28px] bg-lago-cream p-10 text-center"><RefreshCw className="mx-auto h-8 w-8 text-lago-clay" /><h2 className="display mt-4 text-3xl font-semibold">Chưa thể tải lịch trống</h2><p className="mt-3 text-sm text-lago-ink/58">Bạn có thể thay đổi ngày để thử lại hoặc gọi trực tiếp cho Lago.</p><button type="button" onClick={() => setCheckOut(format(addDays(parseISO(checkOut), 1), "yyyy-MM-dd"))} className="btn-primary mt-6">Thử kiểm tra lại</button></div>}

      {availabilityState === "ready" && available.length > 0 && <div className="mt-10 grid gap-5">{available.map((stay, index) => <article key={stay.id} className="result-enter group lift grid overflow-hidden rounded-[28px] border border-lago-ink/10 bg-white lg:grid-cols-[310px_1fr_auto]" style={{ animationDelay: `${index * 70}ms` }}><div className="relative min-h-64 bg-cover bg-center lg:min-h-0" style={{ backgroundImage: `linear-gradient(to top, rgba(10,34,28,.55), transparent 60%), url(${stay.image})` }}><span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-[.6rem] font-bold uppercase tracking-wider">{stay.badge}</span><span className="absolute bottom-4 left-4 flex items-center gap-1.5 text-xs font-semibold text-white"><MapPin className="h-4 w-4 text-lago-sand" />{stay.location}</span></div><div className="p-6 sm:p-7"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-emerald-500" /><p className="text-[.62rem] font-bold uppercase tracking-wider text-emerald-700">Còn trống · Đã kiểm tra</p></div><p className="eyebrow mt-4 text-lago-moss">{stay.subtitle}</p><h2 className="display mt-2 text-3xl font-semibold sm:text-4xl">{stay.name}</h2><p className="mt-3 max-w-xl text-sm leading-6 text-lago-ink/58">{stay.description}</p><div className="mt-5 flex flex-wrap gap-4 text-xs font-semibold text-lago-ink/58"><span className="flex items-center gap-2"><House className="h-4 w-4" />Căn riêng</span><span className="flex items-center gap-2"><Users className="h-4 w-4" />Tối đa {stay.maxGuests} khách</span><span className="text-lago-clay">{stay.highlights[0]}</span></div></div><div className="flex items-center justify-between border-t border-lago-ink/10 p-6 lg:w-56 lg:flex-col lg:items-end lg:justify-center lg:border-l lg:border-t-0"><div className="lg:text-right"><span className="text-xs text-lago-ink/45">{stay.quote.nights} đêm · tổng dự kiến</span><p className="mt-1 text-xl font-bold">{formatCurrency(stay.quote.total)}</p><span className="mt-1 block text-[.65rem] text-lago-ink/38">Đã gồm phụ thu theo lựa chọn</span></div><button type="button" onClick={() => choose(stay)} className="btn-primary lg:mt-6">Chọn căn <ArrowRight className="h-4 w-4" /></button></div></article>)}</div>}

      {availabilityState === "ready" && available.length === 0 && <div className="mt-10 rounded-[28px] bg-lago-cream p-10 text-center"><Users className="mx-auto h-8 w-8 text-lago-clay" /><h2 className="display mt-4 text-3xl font-semibold">Chưa có căn trống cho lựa chọn này</h2><p className="mt-3 text-sm text-lago-ink/58">Hãy thử lùi ngày đi, giảm số khách hoặc liên hệ Lago để được tư vấn đặt nhiều căn.</p><a href="tel:0900000000" className="btn-primary mt-6">Gọi Lago tư vấn</a></div>}
    </>}

    {step === 2 && selected && <div className="grid gap-10 lg:grid-cols-[1fr_390px]"><div><button type="button" onClick={() => setStep(1)} className="focus-ring mb-7 flex items-center gap-2 rounded-lg text-sm font-bold"><ArrowLeft className="h-4 w-4" /> Chọn lại căn</button><p className="eyebrow text-lago-clay">Thông tin liên hệ</p><h1 className="display mt-3 text-5xl font-semibold">Lago sẽ liên hệ với ai?</h1><p className="mt-4 text-lago-ink/60">Chưa cần thanh toán. {selected.name} sẽ được giữ 2 giờ trong lúc đội ngũ Lago xác nhận.</p><form onSubmit={submit} className="mt-8 grid gap-5 sm:grid-cols-2"><label className="text-sm font-bold sm:col-span-2">Họ và tên *<input required name="fullName" autoComplete="name" className="input mt-2" placeholder="Nguyễn Minh Anh" /></label><label className="text-sm font-bold">Số điện thoại / Zalo *<input required name="phone" type="tel" inputMode="tel" autoComplete="tel" className="input mt-2" placeholder="090 123 4567" /></label><label className="text-sm font-bold">Email (không bắt buộc)<input name="email" type="email" autoComplete="email" className="input mt-2" placeholder="ban@email.com" /></label><label className="text-sm font-bold sm:col-span-2">Lời nhắn<textarea name="note" className="input mt-2 h-28 py-3" placeholder="Giờ đến dự kiến hoặc yêu cầu đặc biệt..." /></label><label className="flex items-start gap-3 text-sm leading-6 sm:col-span-2"><input required name="consent" type="checkbox" className="mt-1" />Tôi đồng ý để Lago sử dụng thông tin này nhằm xử lý yêu cầu đặt căn.</label>{error && <p role="alert" className="rounded-xl bg-red-50 p-4 text-sm font-semibold text-red-700 sm:col-span-2">{error}</p>}<button disabled={loading || availabilityState === "loading"} className="btn-primary sm:col-span-2">{loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><ShieldCheck className="h-5 w-5" /> Gửi yêu cầu giữ căn</>}</button></form></div><aside className="card h-fit overflow-hidden lg:sticky lg:top-28"><div className="h-48 bg-cover bg-center" style={{ backgroundImage: `url(${selected.image})` }} /><div className="p-6"><div className="flex items-center gap-2 text-[.62rem] font-bold uppercase tracking-wider text-emerald-700"><span className="h-2 w-2 rounded-full bg-emerald-500" /> Lịch trống đã xác nhận</div><p className="eyebrow mt-4 text-lago-moss">{selected.location}</p><h2 className="display mt-1 text-3xl font-semibold">{selected.name}</h2><DateRangePicker className="mt-5" checkIn={checkIn} checkOut={checkOut} onChange={(range) => { setCheckIn(range.checkIn); setCheckOut(range.checkOut); }} /><p className="mt-3 text-sm text-lago-ink/60">{guests} khách</p><div className="mt-6 space-y-3 border-y border-lago-ink/10 py-5 text-sm"><div className="flex justify-between"><span>{selected.quote.nights} đêm</span><span>{formatCurrency(selected.quote.base)}</span></div>{selected.quote.weekend > 0 && <div className="flex justify-between"><span>Điều chỉnh theo ngày</span><span>{formatCurrency(selected.quote.weekend)}</span></div>}{selected.quote.extraGuest > 0 && <div className="flex justify-between"><span>Phụ thu khách</span><span>{formatCurrency(selected.quote.extraGuest)}</span></div>}</div><div className="mt-5 flex justify-between"><strong>Tổng dự kiến</strong><strong className="text-xl">{formatCurrency(selected.quote.total)}</strong></div><p className="mt-5 flex gap-2 rounded-xl bg-lago-cream p-3 text-xs leading-5"><Clock3 className="h-4 w-4 shrink-0" />Giá được lưu tại thời điểm gửi yêu cầu. Lago sẽ xác nhận mức cuối cùng khi liên hệ.</p></div></aside></div>}
  </div>;
}
