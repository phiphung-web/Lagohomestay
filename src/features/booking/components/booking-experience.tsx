    ...
    const loadAvailability = async () => {
      ...
      try {
        ...
        if (selected) {
          ...
          else setError("Căn bạn quan tâm chưa còn trống trong khoảng ngày này. Lago đã hiển thị các lựa chọn phù hợp khác.");
        }
      } catch (fetchError) {
    ...
    {step === 2 && selected && <div className="booking-step-enter grid gap-10 lg:grid-cols-[1fr_390px]">
      <div><button type="button" onClick={() => setStep(1)} className="focus-ring mb-7 flex items-center gap-2 rounded-lg text-sm font-bold"><ArrowLeft className="h-4 w-4" /> Chọn lại căn</button><p className="eyebrow text-lago-clay">Thông tin liên hệ</p><h1 className="display mt-3 text-5xl font-semibold">Lago sẽ liên hệ với ai?</h1><p className="mt-4 text-lago-ink/60">Chưa cần thanh toán. {selected.name} sẽ được giữ 2 giờ trong lúc đội ngũ Lago xác nhận.</p>
        <form noValidate aria-busy={loading} onSubmit={submit} className="mt-8 grid gap-5 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2"><label className="flex items-start gap-3 text-sm leading-6"><input name="consent" type="checkbox" onChange={() => clearFieldError("consent")} aria-invalid={Boolean(fieldErrors.consent)} aria-describedby={fieldErrors.consent ? "booking-consent-error" : undefined} className="mt-1" />Tôi đồng ý để Lago sử dụng thông tin này nhằm xử lý yêu cầu đặt căn.</label>{fieldErrors.consent && <p id="booking-consent-error" className="ml-8 mt-2 text-xs font-semibold text-red-700">{fieldErrors.consent}</p>}</div>
