...
function CalendarMonth({
  ...
  const days = eachDayOfInterval({
    ...
    end: startOfWeek(addMonths(endOfMonth(month), 1), { weekStartsOn: 1 })
  });
...
export function DateRangePicker({ checkIn, checkOut, onChange, className = "", tone = "light" }: Props) {
  ...
  return <div className={className}>
    ...
    {open && <div className="fixed inset-0 z-[80] flex items-end bg-lago-ink/65 p-0 backdrop-blur-sm sm:items-center sm:justify-center sm:p-5" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section role="dialog" aria-modal="true" aria-label="Chọn ngày lưu trú" className="calendar-sheet w-full max-w-3xl rounded-t-[30px] bg-white p-5 shadow-2xl sm:rounded-[30px] sm:p-7">
        ...
        <div className="my-5 flex items-center justify-between">
          <button type="button" onClick={() => setViewMonth((month) => addMonths(month, -1))} aria-label="Tháng trước" className="focus-ring grid h-10 w-10 place-items-center rounded-full border border-lago-ink/10"><ChevronLeft className="h-4 w-4" /></button>
        ...
        <footer className="mt-6 flex items-center justify-between gap-4 border-t border-lago-ink/10 pt-5">
