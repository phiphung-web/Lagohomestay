...
export function AvailabilityBar({ compact = false }: { compact?: boolean }) {
  ...
  return <form onSubmit={submit} className={`grid gap-3 ${compact ? "lg:grid-cols-[1fr_1fr_.75fr_auto]" : "rounded-[24px] bg-white p-4 shadow-soft md:grid-cols-[1fr_1fr_.75fr_auto]"}`}>
    <label className="group rounded-xl border border-lago-ink/10 bg-white px-4 py-2"><span className="flex items-center gap-2 text-[.65rem] font-bold uppercase tracking-wider text-lago-ink/50"><CalendarDays className="h-3.5 w-3.5"/>Ngày đến</span><input aria-label="Ngày đến" min={format(today, "yyyy-MM-dd")} value={checkIn} onChange={(e) => setCheckIn(e.target.value)} type="date" className="mt-1 w-full bg-transparent font-semibold outline-none" required/></label>
    <label className="rounded-xl border border-lago-ink/10 bg-white px-4 py-2"><span className="flex items-center gap-2 text-[.65rem] font-bold uppercase tracking-wider text-lago-ink/50"><CalendarDays className="h-3.5 w-3.5"/>Ngày về</span><input aria-label="Ngày về" min={checkIn} value={checkOut} onChange={(e) => setCheckOut(e.target.value)} type="date" className="mt-1 w-full bg-transparent font-semibold outline-none" required/></label>
    ...
    <button className="btn-primary min-w-[168px]" type="submit"><Search className="h-4 w-4"/>Tìm căn phù hợp</button>
  </form>;
}
