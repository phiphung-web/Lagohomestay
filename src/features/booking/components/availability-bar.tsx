...
export function AvailabilityBar({ compact = false, bookingPath = "/lien-he#inquiry-form", locale = "vi" }: { compact?: boolean; bookingPath?: string; locale?: ShowcaseLocale }) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    const [path, hash] = bookingPath.split("#");
    router.push(`${path}?checkIn=${checkIn}&checkOut=${checkOut}&guests=${guests}${hash ? `#${hash}` : ""}`);
  };
  ...
  return <form onSubmit={submit} className={`grid items-stretch gap-3 ${compact ? "lg:grid-cols-[1.5fr_210px_auto]" : "rounded-[24px] bg-white p-3 shadow-soft md:grid-cols-[1.5fr_210px_auto]"}`}>
    ...
    <button className="btn-primary min-w-[178px]" type="submit"><Search className="h-4 w-4" /><span>{locale === "en" ? "Contact LAKA" : "Liên hệ LAKA"}</span><small className="font-medium text-white/60">· {nights} {locale === "en" ? (nights === 1 ? "night" : "nights") : "đêm"}</small></button>
  </form>;
}
