...
**API đặt phòng:** availability / bookings / lookup dùng dữ liệu bộ nhớ khi `DEMO_MODE` khác `"false"` hoặc thiếu `DATABASE_URL`. Khi `DEMO_MODE="false"` và có kết nối database, API dùng Prisma. Các API này độc lập với form tư vấn; repo hiện không có giao diện quản trị booking.
...
Các route `/admin/*`, `/api/admin/*` và `/api/auth/*` đã được gỡ cùng toàn bộ mã đăng nhập/session. Model `User` và enum `UserRole` còn trong schema để tương thích database và lịch sử migration; ứng dụng không đọc chúng, seed không tạo tài khoản. Lần gỡ admin không có migration xóa bảng hoặc dữ liệu.
...
- `server` chứa hạ tầng chỉ chạy phía server. Client Component không import Prisma.
