...
import {
  ...
  createBookingSchema,
  lookupBookingSchema,
} from "@/features/booking/domain/validation";
import {
  validateBookingContact,
  validateLookupPhone,
} from "@/features/booking/domain/contact-validation";
...
describe("booking validation", () => {
  ...
  it("từ chối ngày trả trước ngày nhận", () => {
    expect(
      availabilitySchema.safeParse({ checkIn: "2026-08-03", checkOut: "2026-08-01", guests: 2 })
        .success,
    ).toBe(false);
  });
  ...
  it("trả lỗi theo từng trường để giao diện không dùng thông báo mặc định của trình duyệt", () => {
    expect(
      validateBookingContact({ fullName: "A", phone: "abc", email: "sai-email", consent: false }),
    ).toEqual({
